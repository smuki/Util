...
namespace Volte.Utils
{
    ...
    {
        const string ZFILE_NAME = "CronExpression";
		/// <summary>
            ...
            try
            {
                ...
                if ((c >= 'A') && (c <= 'Z') && (!s.Equals("L")) && (!s.Equals("LW"))) {
                    ...
                    if (type == Month) {
                        ...
                        throw new FormatException(string.Format("Illegal characters for this position: '{0}'", sub));
                    }
                    if (eval != -1) {
                        incr = 1;
                    }
                ...
                if (c == '*' || c == '/')
                {
                    ...
                    if (c == '/') {
                        ...
                        {
                            //Console.WriteLine("A14332-");
                            throw new FormatException( string.Format( "Increment > 60 : {0}", incr));
                        }
                        else if (incr > 23 && (type == Hour))
                        {
                            //Console.WriteLine("A143332-");
                            throw new FormatException( string.Format( "Increment > 24 : {0}", incr));
                        }
                        else if (incr > 31 && (type == DayOfMonth))
                        {
                            //Console.WriteLine("A1423332-");
                            throw new FormatException( string.Format( "Increment > 31 : {0}", incr));
                        }
                        else if (incr > 7 && (type == DayOfWeek))
                        {
                            //Console.WriteLine("A1423332-");
                            throw new FormatException( string.Format( "Increment > 7 : {0}", incr));
                        }
        ...
        protected virtual int CheckNext(int pos, string s, int val, int type)
        {
            ...
            if (c == '#') {
                if (type != DayOfWeek) {
                    throw new FormatException( string.Format( "'#' option is not valid here. (pos={0})", i));
                }
                i++;
                try
                {
